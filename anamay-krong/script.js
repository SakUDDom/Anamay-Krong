...
// 🚀 Optimization: ទាញយកតែទិន្នន័យចាំបាច់ (លុប photo_base64)
async function fetchAndRenderData() {
    try {
        const { data: households, error: hError } = await supabaseClient.from('households')
            .select('id, custom_id, customer_name, name, lat, lng, zone, status_color, monthly_fee, payment_month, photo_url');
